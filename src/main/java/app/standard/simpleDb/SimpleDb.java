...
public class SimpleDb {
    ...
    private Connection getCurrentThreadConnection() {
        ...
        try {
            ...
            if (conn == null) {
                Connection curThreadConn = DriverManager.getConnection(dbUrl, dbUser, dbPassword);
                connections.put(Thread.currentThread().getName(), curThreadConn);

                return curThreadConn;
            }
    ...
    public <T> List<T> selectRows(String sql, List<Object> params, Class<T> cls) {
        return selectRows(sql, params).stream()
                .map(map -> {
                    return Util.Mapper.mapToObj(map, cls);
                }).toList();
    }
